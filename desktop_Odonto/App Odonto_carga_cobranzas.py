...
query_presupuestos_fecha= App_Odonto_SQL.query_presupuestos_fecha    # Trae las fechas de los presupuestos.
#query_id=App_Odonto_SQL.query_id                  # Trae el Id del registro según Nombre o DNI de la tabla 'pacientes'.

query_nombre_dni=App_Odonto_SQL.query_nombre_dni    # Trae el nombre del paciente según el DNI consultado.
query_dni_nombre=App_Odonto_SQL.query_dni_nombre    # Trae el DNI del paciente según el Nombre consultado.

query_tratamiento= App_Odonto_SQL.query_tratamiento   # Trae el tratamiento del paciente según el Id consultado.
query_presupuesto= App_Odonto_SQL.query_presupuesto       # Trae el presupuesto del paciente según el Id consultado.
query_id_cobranzas= App_Odonto_SQL.query_id_cobranzas    # Trae el Id del registro según Nombre o DNI de la tabla 'cobranzas'.
query_deuda = App_Odonto_SQL.query_deuda              # Trae la deuda del paciente según el Id consultado.

crear_cobro= App_Odonto_SQL.crear_cobro    # Carga datos en la tabla 'cobranzas'.
...
cobros_col= [
                [sg.Text('Buscar por D.N.I',size=(20,1),font=(formatos['Letra'],formatos['Tamaño título']))], #Consultar DNI del paciente para cargarle un cobro.
                ...
                [sg.Text('Buscar por Nombre',size=(20,1),font=(formatos['Letra'],formatos['Tamaño título']))], # Consultar nombre del paciente para cargarle un cobro.
                ...
                [sg.Text('Buscar por Fecha',size=(20,1),font=(formatos['Letra'],formatos['Tamaño título']))], # Consultar fecha del presupuesto para cargarle un cobro.
                [sg.Combo(values=(query_presupuestos_fecha()),default_value='Seleccionar',size=(15,1),font=(formatos['Letra'][0],11),enable_events=True,key='-Lista FECHA-')],    
...
while True:
    ...
    fechas_presupuestos = query_presupuestos_fecha () # Trae las fechas de los presupuestos.

    window_cobros['-Lista NOM-'].update(values=nombre_pacientes)
    window_cobros['-Lista DNI-'].update(values=dni_pac)
    window_cobros['-Lista FECHA-'].update(values=fechas_presupuestos)
    ...
    if event == '-Lista DNI-':
        ...
        window_cobros['-Nombre-'].update(nombre_dni[0])
        window_cobros['-Lista DNI-'].update(dni_unico)    
        window_cobros['-Lista NOM-'].update(nombre_dni[0])
    ...
    if event == '-Lista NOM-':
        ...
        window_cobros['-DNI-'].update(dni_nombre[0])
        window_cobros['-Lista DNI-'].update(dni_nombre[0])
        window_cobros['-Lista NOM-'].update(nombre_prolijo)
        #window_cobros['-TRA-'].update(tratamiento[0])
        #window_cobros['-PRECIO-'].update(presupuesto[0])
        #window_cobros['-DEUDA-'].update(deuda[0])

    if event == '-Lista FECHA-':
        dni_unico=(values['-Lista DNI-'])
        window_cobros['-DNI-'].update(dni_unico)
