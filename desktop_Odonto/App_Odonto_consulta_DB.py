...
connection = App_Odonto_SQL.connection
query_info = App_Odonto_SQL.query_info            # Trae el nombre, teléfono y mail de los pacientes.
...
query_historia= App_Odonto_SQL.query_historia             # Trae la hsitoria clínica de los pacientes.
#query_costo=App_Odonto_SQL.query_costo            # Trae el costo de los pacientes.
query_presupuesto = App_Odonto_SQL.query_presupuesto          # Trae el presupuesto de los pacientes.
query_forma_de_pago= App_Odonto_SQL.query_forma_de_pago           # Trae la forma de pago de los pacientes.
...
col_izquierda=[
                [sg.Text('Consultar por D.N.I',size=(20,1),font=(formatos['Letra'],formatos['Tamaño título']))],[sg.Combo(values=(query_pacientes_dni()),default_value='Seleccionar',font=(formatos['Letra'][0],11),enable_events=True,key='-PAC-')],
                [sg.Text('Consultar por Nombre',size=(20,1),font=(formatos['Letra'],formatos['Tamaño título']))],[sg.Combo(values=(query_pacientes_nombre()),default_value='Seleccionar',font=(formatos['Letra'][0],11),enable_events=True,key='-NOM-',size=(15,1))],                
                [sg.HorizontalSeparator(pad=(0,10))],
                [sg.Text('Info contacto', size=(18,1),font=(formatos['Letra'],formatos['Tamaño título'],'underline'),justification='center',enable_events=True,key='-TEXT-')],
                [sg.Text('Nombre' +' - '+ 'Mail' +' - '+ 'DNI', size=(25,1),pad=(5,10),font=(formatos['Letra'][0],formatos['Tamaño bloques'],formatos['Letra'][1]),key='-INFO-')],
...
layout_consulta_pacientes=[
                            [sg.Text("Consulta de Pacientes",size=(20,1),font=(formatos['Letra'][0],formatos['Tamaño título'],formatos['Letra'][1]))],
                            [sg.Column(col_izquierda,element_justification='l',expand_x=True,vertical_alignment='t',grab=True), 
...
while True:
    ...
    if event == '-NOM-':
        ...
        pacientes_nombre=(values['-NOM-'])
        print(pacientes_nombre)
        nombre=f'"{pacientes_nombre}"' # Le quita las llaves al texto.
